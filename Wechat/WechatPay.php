...
class WechatPay
{
    ...
    public function getPrepayId($data = '') {
        if (!$data['body']) die('缺少统一支付接口必填参数body！');
        if (!$data['out_trade_no']) die('缺少统一支付接口必填参数out_trade_no！');
        if (!$data['total_fee']) die('缺少统一支付接口必填参数total_fee！');
        if (!$data['trade_type']) die('缺少统一支付接口必填参数trade_type！');
        ...
        empty($data['openid']) || $postdata['openid'] = $data['openid'];
        empty($data['scene_info']) || $postdata['scene_info'] = $data['scene_info'];
    ...
    public function getPrepayId1($openid, $body, $out_trade_no, $total_fee, $notify_url, $trade_type = "JSAPI", $goods_tag = null, $fee_type = 'CNY') {
        $postdata = array(
            "body" => $body,
            "out_trade_no" => $out_trade_no,
            "fee_type" => $fee_type,
        ...
        if ($trade_type === 'MWEB') {
            $postdata['scene_info'] = '{"h5_info": {"type":"Wap","wap_url": "http://api.biteabc.cn","wap_name": "腾讯充值"}}';
        }
